...
#define LED_PIN    21      // Data pin connected to DIN
...
void setAllWhite(uint8_t brightness) {
  // Set all LEDs to white with the same brightness level
...
void enable_led(bool en) {  // Turn LED Ring On or Off
  int duty = en ? led_duty : 0;
  if (en && isStreaming && (led_duty > CONFIG_LED_MAX_INTENSITY)) {
    duty = CONFIG_LED_MAX_INTENSITY;
  }
  setAllWhite(duty);
  log_i("Set LED Ring intensity to %d", duty);
}
...
void setupLedFlash() {
  ...
  log_i("NeoPixel LED Ring initialized on pin %d with %d LEDs", LED_PIN, LED_COUNT);
}
